...
class TorchMinMaxObserver(BaseTensorObserver):
    def __init__(self, watch_on: Variable, quant_cfg: TensorQuantizationConfig):
        ...
        self._min_val_collator = []
        self._max_val_collator = []
    ...
    def observe(self, value: torch.Tensor):
        ...
        if self._quant_cfg.state == QuantizationStates.INITIAL:
            if self._quant_cfg.policy.has_property(QuantizationProperty.PER_TENSOR):
                self._min_val_collator.append(value.min().reshape(shape=[1, ]))
                self._max_val_collator.append(value.max().reshape(shape=[1, ]))
                ...
                self._min_val_collator.append(torch.min(channelwise_view, dim=1, keepdim=True)[0])
                self._max_val_collator.append(torch.max(channelwise_view, dim=1, keepdim=True)[0])
            else:
    ...
    def render_quantization_config(self):
        if len(self._max_val_collator) == 0:
        ...
        device = self._max_val_collator[-1].device

        if self._quant_cfg.policy.has_property(QuantizationProperty.PER_TENSOR):
            scale, offset = self.minmax_to_scale_offset(
                min_val = torch.min(torch.cat(self._min_val_collator, dim=0)).cpu().item(),
                max_val = torch.max(torch.cat(self._max_val_collator, dim=0)).cpu().item(),
            ...
            min_vals = torch.min(torch.cat(self._min_val_collator, dim=-1), dim=-1, keepdim=False)[0].cpu()
            max_vals = torch.max(torch.cat(self._max_val_collator, dim=-1), dim=-1, keepdim=False)[0].cpu()
...
class TorchHistObserver(TorchMinMaxObserver):
    def __init__(self, watch_on: Variable, quant_cfg: TensorQuantizationConfig, hist_bins: int = 8192):
        self._phase = 'Detecting Minmax'
        self._hist  = None
        self._hist_scale = None
        self._hist_bins  = hist_bins
        super().__init__(watch_on, quant_cfg)
        if not quant_cfg.policy.has_property(QuantizationProperty.SYMMETRICAL):
            ppq_warning('Applying hist observer with your tensor will make offset = 0'
                '(However it is an ASYMMETRICAL quantized tensor)')
        if not quant_cfg.policy.has_property(QuantizationProperty.PER_TENSOR):
    ...
    def hist_to_scale_offset(
        ...
        config: TensorQuantizationConfig, computing_device: str = 'cpu',
        scale_threshold: float=1e-7
    ) -> Tuple[float, int]:
        """
        ...
        losses, quant_bins = [], (config.quant_max - 0 + 1)
...
class TorchPercentileObserver(TorchMinMaxObserver):
    def __init__(self, watch_on: Variable, quant_cfg: TensorQuantizationConfig):
        super().__init__(watch_on, quant_cfg)
        if 'percentile' in quant_cfg.detail:
            self._percentile = quant_cfg.detail['percentile']
        else:
            # TODO carefully search this.
            self._percentile = 0.9999
