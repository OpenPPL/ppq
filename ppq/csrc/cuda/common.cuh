...
float QuantizeScalarFloating(
    ...
    /* Following formula will enlarge exponent part    */
    /* fp32_exp += 1, fp32_mantissa >>= 1             */
    /* Hopefully, we can simply do this by fp32 *= 2  */
	if ((fp32_exp - 127) < exponent_min){
		uint32_t shift = (exponent_min - fp32_exp + 127);
		fp32_exp       += shift;
		fp32_mantissa >>= shift;
	}
    ...
    rounding_helper.data = (fp32_mantissa << (23 - mantissa) & 0x007FFFFF) + 0x3F800000;
    uint32_t round_bit = _round2int(rounding_helper.value, rounding);
    ...
    // process overflow
    if (helper.value >= clip_max) return clip_max;
    if (helper.value <= clip_min) return clip_min;
    return helper.value + offset;
}
