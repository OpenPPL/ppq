...
unquantized = load_graph(file_path='Output/onnx.model')

for name, _ in unquantized.operations.items():
    # randomly dispatch operations to FP32 platform:
    if random.random() > 0.5:
        quant_setting.dispatching_table.append(name, TargetPlatform.FP32)
        print(f'Operation {name} is dispatching to FP32.')
