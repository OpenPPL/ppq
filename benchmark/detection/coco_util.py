...
def evaluate_onnx_module_with_coco(
    ...
    with torch.no_grad():
        ...
        #     input_feed={input_placeholder_name: input_tensor.cpu().numpy()}, output_names=None))[0]
        return _evaluate_any_module_with_coco(
...
def evaluate_openvino_module_with_coco(
...
def evaluate_trt_module_with_coco(
    model_path: str, imagenet_validation_dir: str=None,
    batchsize: int = 32, device: str = 'cuda',
    imagenet_validation_loader: DataLoader = None,
    verbose: bool = True
...
def _evaluate_any_module_with_coco(
